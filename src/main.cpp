...
#if defined(NDEBUG)
# error "Zephside cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdd3c7a80a79dd8c39e9f0eb3fa0428c429388d0535ee78123caf367d441ea19e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zephside: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zephside Signed Message:\n";
...
{
    // Zephside: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Zephside: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Zephside: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Zephside: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zephside: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zephside: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdd3c7a80a79dd8c39e9f0eb3fa0428c429388d0535ee78123caf367d441ea19e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Your Mom Is Dead";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047307e41461d80081f603ceba30eb10ab6d2135197c6d711814e592e0b8d608ece46f60744279f5d5f80b0e8775e042a55ee80e8330f946dd4fcb33ec1ff2fc79") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523593281;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084927661;

        if (fTestNet)
        {
            block.nTime    = 1523593222;
            block.nNonce   = 2084927661;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe353347f78fce2894d3e61b9cd73a07db26bb03f96fd11780fa3f566083bbbaa"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb4, 0xd3 }; // Zephside: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZephsideMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZephsideMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZephsideMiner(CWallet *pwallet)
{
    printf("ZephsideMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zephside-miner");
    ...
    try { loop {
        ...
        printf("Running ZephsideMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZephsideMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZephsideMiner, pwallet));
}
